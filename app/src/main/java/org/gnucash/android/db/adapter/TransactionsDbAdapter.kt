...
class TransactionsDbAdapter(
        ...
        try {
            ...
            if (didChange) {
                val deleteWhere = (SplitEntry.COLUMN_TRANSACTION_UID + " = ? AND "
                        + SplitEntry.COLUMN_UID + " NOT IN " +
                        splitUIDs.joinToString("','", prefix = "('", postfix = "')"))
                val deleteArgs = arrayOf<String?>(transaction.getUID())
    ...
    override fun bind(stmt: SQLiteStatement, transaction: Transaction): SQLiteStatement {
        ...
        stmt.bindString(6, transaction.commodity.getUID())
    ...
    override fun buildModelInstance(cursor: Cursor): Transaction {
        ...
        transaction.splits = splitsDbAdapter.getSplitsForTransaction(transaction.getUID())
