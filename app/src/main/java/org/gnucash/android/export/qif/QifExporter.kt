 ...
 */
class QifExporter @JvmOverloads constructor(
    context: Context,
    params: ExportParams,
    bookUID: String,
    ...
    override fun writeExport(writer: Writer, exportParams: ExportParams) {
        val lastExportTimeStamp = mExportParams.exportStartTime.time.toString()
        val transactionsDbAdapter = mTransactionsDbAdapter

        val accountsList = mAccountsDbAdapter.simpleAccounts
        ...
        val where = TransactionEntry.TABLE_NAME + "_" + TransactionEntry.COLUMN_TEMPLATE + " == 0 AND " +
        ...
        try {
            ...
            do {
                ...
                val time = cursor.getLong(cursor.getColumnIndexOrThrow("trans_time"))
                val description = cursor.getString(cursor.getColumnIndexOrThrow("trans_desc"))
                ...
                val splitCount = cursor.getInt(cursor.getColumnIndexOrThrow("trans_split_count"))

                val account1: Account = checkNotNull(accounts[accountUID])
                ...
                if (transactionUID != currentTransactionUID) {
                    ...
                    if (!decimalImbalance.isZero) {
                        writer.append(SPLIT_CATEGORY_PREFIX)
                            .append('[')
                            .append(
                                AccountsDbAdapter.getImbalanceAccountName(mContext, commodity)
                            )
                            .append(']')
                            .append(NEW_LINE)
                ...
                // all splits
                val account2UID = cursor.getString(cursor.getColumnIndexOrThrow("acct2_uid"))
                val account2: Account = checkNotNull(accounts.get(account2UID))
                ...
                val quantity_denom = cursor.getDouble(cursor.getColumnIndexOrThrow("split_quantity_denom"))
                ...
                var quantity = if (quantity_denom != 0.0) {
                    BigDecimal.valueOf(quantity_num).divide(BigDecimal.valueOf(quantity_denom))
                } else {
                    BigDecimal.ZERO
                }
