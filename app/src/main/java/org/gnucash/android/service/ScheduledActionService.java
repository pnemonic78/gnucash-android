...
public class ScheduledActionService {
    ...
    private static void executeScheduledEvent(@NonNull DatabaseHolder dbHolder, @NonNull ScheduledAction scheduledAction) {
        ...
        switch (scheduledAction.getActionType()) {
            case TRANSACTION:
                instanceCount += executeTransactions(dbHolder, scheduledAction);
                break;

            case EXPORT:
                instanceCount += executeBackup(dbHolder, scheduledAction);
                break;
        }
        ...
        if (instanceCount > 0) {
            ...
            scheduledAction.setInstanceCount(instanceCount);
            // Update the last run time and execution count
            ContentValues contentValues = new ContentValues();
            contentValues.put(ScheduledActionEntry.COLUMN_LAST_OCCUR,
                scheduledAction.getLastRunTime());
            contentValues.put(ScheduledActionEntry.COLUMN_INSTANCE_COUNT,
                scheduledAction.getInstanceCount());
            SQLiteDatabase db = dbHolder.db;
            db.update(ScheduledActionEntry.TABLE_NAME, contentValues,
                ScheduledActionEntry.COLUMN_UID + "=?", new String[]{scheduledAction.getUID()});
        }
    }
    ...
    private static boolean shouldExecuteScheduledBackup(ScheduledAction scheduledAction) {
        if (scheduledAction.getActionType() != ScheduledAction.ActionType.EXPORT) {
            return false;
        }
        long now = System.currentTimeMillis();
