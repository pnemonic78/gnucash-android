 ...
 */
class GncXmlHandler @JvmOverloads constructor(
    ...
    private fun setMinimalScheduledActionByDays() {
        ...
        scheduledAction.recurrence!!.byDays = listOf<Int>(calendar.get(Calendar.DAY_OF_WEEK))
    }
    ...
    private fun handleEndAccount(uri: String, value: String) {
        if (NS_GNUCASH == uri) {
            ...
            if (isInTemplates) {
                ...
                split!!.setValue(split!!.value.withCommodity(transaction!!.commodity))
            }
        }
    }
    ...
    private fun handleEndSlotTemplateFormula(
        ...
        try {
            ...
            if (split.value.isAmountZero) {
                ...
                val amount = Money(value, commodity)
    ...
    private fun handleEndSlotTemplateNumeric(
        ...
        try {
            ...
            if (split.value.isAmountZero) {
                ...
                val commodity = getCommodityForAccount(accountUID)
                val amount = Money(splitAmount, commodity)
    ...
    private fun handleEndValue(uri: String, value: String) {
        if (NS_PRICE == uri) {
            ...
            try {
                ...
                val amount = parseSplitAmount(value).abs() // use sign from quantity
                split.setValue(Money(amount, Commodity.DEFAULT_COMMODITY))
            } catch (e: ParseException) {
                val msg = "Invalid split quantity $value"
                throw SAXException(msg, e)
            }
